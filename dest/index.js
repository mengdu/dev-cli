...
_commander2.default.name('dev').version(_package2.default.version).description('A devlopment tool').usage('<command> [options]').option('-i, --info', 'Tool introduction').command('serve', 'Create a local static file server').alias('s').command('fanyi', 'Translate tool').alias('f').parse(process.argv);
